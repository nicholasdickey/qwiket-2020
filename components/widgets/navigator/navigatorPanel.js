...
export let NavigatorPanel = ({
    ...
    console.log("NavigatorPanel:", {
        navigatorData: navigatorData ? navigatorData.toJS() : {},
    });
    ...
    if (navigatorType == "definedTags") {
        ...
        console.log({
            navigatorTree: navigatorTree ? navigatorTree.toJS() : {},
            definedTags,
            navigatorTag,
        });
    }
    ...
    let TagSelect = ({ tags, currentTag }) => {
        ...
        let selector = tags.map(t => (
            <MenuItem value={t.tag}>{t.text}</MenuItem>
        ));
        let SelectC = () => (
            <Select
        ...
        return (
            <Styles>
                {" "}
                <FormControl
                    classes={{ root: "metatag-selector" }}
                    variant="outlined"
                    fullWidth>
                    <FormLabel
                        className="metatag-selector-label"
    ...
    let treeRenderer = ({ session, ...other }) => {
        ...
        return (
            <TabsNavigator
                session={session}
                {...other}
                leftRenderer={innerTreeRenderer}
            />
        );
    };
    let innerListRenderer = ({
        navigatorData,
        tags,
        currentTag,
        session,
