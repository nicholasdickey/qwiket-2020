...
import LinkPopup from "./linkPage";

let RenderTag = React.memo(
    ({
        channel,
        shortname,
        parentName,
        ...
        let LeftWrap = styled.div`
            ...
            padding-right: 6px;
        `;
        const OuterLink = styled(({ ...other }) => (
            <div>
                {metaLink.external ? (
                    <a
                        data-id="tag-outer-link-external"
                        href={metLink.link}
                        {...other}
                    />
                ) : (
                    <a data-id="tag-outer-link">
                        <Link
                            route={metaLink.route}
                            params={metaLink.params}
                            {...other}
                        />
                    </a>
                )}
                        ...
                        <ParentLinks>
                            {parentName ? (
                                <Link
                                    ...
                                    params={{
                                        channel,
                                        soloShortname: parentShortname,
                                    }}>
                                    <a>{parentName}</a>
                                </Link>
                            ) : null}
                            <Link route="news" params={{ channel }}>
                                <a>&nbsp;&nbsp;&nbsp;&nbsp;To Newsline</a>
                            </Link>
                        </ParentLinks>
                        <OuterLink>
                            <StyledPaper>
                                {Boolean(image) ? (
                                    <StyledImage responsive src={image} />
                                ) : null}
                                <TitleOuterDiv>
                                    {Boolean(name) ? (
...
let Tag = ({ qparams, context, app, session, actions }) => {
    ...
    if (!shortname) {
        //if explicit shortname is not present in url, then pick the first tag from the topic itself
    ...
    let isIncluded = shortname => {
        return myfeeds
            ? myfeeds.find(feed => feed.shortname == shortname && feed.included)
            : false;
    };
